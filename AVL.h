...
template <typename T> class AVL{
    ...
    /*
    ***************************************
    INORDER AND PREORDER FUNCTION FOR CLASS
    ***************************************
     void inorder(Node* rt){
        if(rt==NULL)return;
        ...
        //     cout<<rt->data<<" ";
        // }
        cout<<"Name: "<<(rt->data).name<<"   Roll No: "<<(rt->data).rollNo<<endl;
        inorder(rt->right);
    }
    ...
    T closest_element(T val){
        T lessEqual = lesserOrEqual(root, val, defaultDT);
        T ub = upper_bound(val);

        if(isEqual(lessEqual,defaultDT)){ //lessEqual==defaultDT
            // val is smaller than smallest of the tree
            // in this case the upper bound will be the smallest value of the tree
            return ub;
        }
        if(isEqual(lessEqual,val)){ //lessEqual==val
            // node with same value as val is present in the tree
            return val;
        }
        if(isEqual(ub, defaultDT)){ //ub==defaultDT
            // ether val is the greatest element of the tree of it is the greater then the greatest element of the terminate
            // in the or case lessEqual will be the greatest element of the tree and that will be the closest so return it
            return lessEqual;
        }

        // now the situation is val is not present in the tree and it is in between the greatest and lowest node of the tree


        T diffa = val - lessEqual;
        T diffb = ub - val;

        if(cmprtr(diffa, diffb) || isEqual(diffa, diffb)) return lessEqual; //diffa <= diffb
        else return ub;
    }
    ...
    while(1){
        cin >> choice;
            ...
            case 9: cin >> eLeft >> eRight;
                    ...
                    // cout<<"#ele in range ["<<eLeft<<" to "<<eRight<<"] is: "<<avlTree->count_range(eLeft,eRight)<<endl;
                    break;
                    break;
...
void printStudentDetails(StudentDetails &sd){
    cout<<"Name: "<<(sd.name)<<"   Roll No: "<<(sd.rollNo)<<endl;
}
    ...
    while(1){
        cin >> choice;
            ...
            case 4:{
                    cin >> id;
                    StudentDetails sd = StudentDetails("", 0, id);
                    cout << avlTree->count_occurence(sd) << endl;
                    break;
            }       
            ...
            case 8:{
                    cin >> index;
                    StudentDetails sd = avlTree->Kth_largest(index);
                    printStudentDetails(sd);
                    break;
            } 
            ...
            case 9:{
                    cin>>eLeft>>eRight;
                    StudentDetails  eL = StudentDetails("", 0, eLeft);
                    StudentDetails eR = StudentDetails("", 0, eRight);
                    cout<<avlTree->count_range(eL,eR)<<endl;
                    // cout<<"#ele in range ["<<eLeft<<" to "<<eRight<<"] is: "<<avlTree->count_range(eLeft,eRight)<<endl;
                    break;
            } 
